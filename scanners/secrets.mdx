...
|---|---|
| `production` | Everything not matched below — full severity |
| `test` | `tests/`, `test/`, `spec/`, `__tests__/`, `e2e/`, `integration/`, `unit/`, `*.test.*`, `*.spec.*`, `test_*.py` |
| `fixture` | `fixtures/`, `mocks/`, `stubs/`, `fakes/`, `seeds/`, `factory/`, `testdata/` |
...
This is not recommended as the default — credentials in test history are still in your git history and still exploitable. See [Context-aware scanning](/cli/scan#context-aware-scanning) for the full rationale.
