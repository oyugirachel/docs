...
Get your API key from the [ZenVeil dashboard](https://app.zenveil.dev/settings/api-keys) and set it as an environment variable:
...
```bash
echo 'export ZENVEIL_API_KEY="zvk_live_your_key_here"' >> ~/.zshrc
source ~/.zshrc
```

Or use `zenveil login` to authenticate interactively — this stores your key in `~/.zenveil/credentials`.
